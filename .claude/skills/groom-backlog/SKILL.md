...
**Empirical blocker — evidence or defer (this repo's discipline).** **Documented ≠ verified.** `docs/ibkr-web-api-spec.md` and IBKR's own docs record *claims about* upstream behavior; a story that **builds on** an upstream behavior no recording or live observation confirms is **not loop-ready**. Close it during grooming, while attended:

- **Live-probe the paper account, same-day:** run `tools/ApiCapture/` for the endpoint(s) in question (recordings land in `recordings/`), or an `[EnvironmentFact]` E2E / example app against the paper credentials. Commit the recording/observation as the evidence the spec cites.
