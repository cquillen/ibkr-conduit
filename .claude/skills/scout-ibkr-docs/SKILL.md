...
1. **Select sources** by walking the registry's `Covers`, `Overlaps`, and scope caveats in `Authority notes` — this is a **judgement call on topical relevance, not a hard rule and not "all sources every time"** (operator, 2026-07-07). Match what the question is actually about against each entry's stated scope and skip sources that plainly don't speak to it (an order-placement question doesn't need DOC-10's entitlements page; a Flex question doesn't need DOC-08's order-type reference; DOC-09's changelog earns a scout only when recency/drift is part of the question). Among the sources that DO plausibly speak to it, prefer covering the question from all of them — single-source answers are what the old mirrors gave us — and when relevance is genuinely uncertain, err toward including the source. Name the skipped sources in one line ("skipped DOC-06/08/09/10 — out of topic") so a bad pruning call is visible. No registered source covers it? Say so; suggest the operator register one (`register-ibkr-doc-source`).
