...
6. **Possible new doc sources** — when the source's **content** defers to or links another IBKR documentation page for material relevant to the question ("refer to the Order Types page", "see our Reference Material", a deep link inside a section body), report it under a `Possible new doc sources:` line: the URL, the exact deferring sentence, and what it apparently covers. Do NOT fetch it (one-source rule) and do NOT harvest site chrome — nav menus, footers, sidebars, and related-article widgets are noise; only in-content, topic-relevant deferrals count. `none` is a fine answer. The dispatcher surfaces these to the operator, who decides whether to register (`register-ibkr-doc-source`); precedent: DOC-08 was found exactly this way.
