...
/// <see cref="string"/>. IBKR's streaming WebSocket frames are inconsistent about whether
/// numeric-looking identifiers (e.g. <c>orderId</c>) are quoted, so a field typed as
/// <see cref="string"/> in a DTO may arrive as either a JSON string or a JSON number.
/// Numbers that fit in a <see cref="long"/> are converted via <see cref="CultureInfo.InvariantCulture"/>;
/// other numbers fall back to their raw JSON text. Writes are always emitted as a JSON string.
/// </summary>
