...
- **Trades' retry is "immediate" in its own code path only — the endpoint rate limiter can still add up to ~5s.** `/iserver/account/trades` carries an endpoint-specific limiter (`1 req/5 secs`, §8.1); `GetTradesAsync`'s retry travels the same rate-limited pipeline as any other call, so it queues behind that limiter's token instead of firing wall-clock-immediately whenever the first call has just consumed the endpoint's sole token — up to a full ~5s replenishment period. Since "looks sparse" is evaluated per call, a live polling consumer pays this stall on every quiet (trade-free) poll, not just the session's first. This is a deliberate tradeoff (ADR-0009 Consequences, 2026-07-14): bypassing the limiter for the retry was rejected because `1 req/5 secs` is IBKR's own documented limit for this endpoint, and exceeding it risks a 429 and the shared, IP-wide 10-minute penalty box (§8.5) — a worse outcome for every tenant than one bounded ~5s stall. Positions is unaffected — `/portfolio/{accountId}/positions/{pageId}` has no registered endpoint limiter today.
