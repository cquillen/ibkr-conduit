...
### 10.7 Positions/Trades Cold-Read Handling

**[ADR-0009](adr/0009-positions-trades-cold-read-retry.md) (revised 2026-07-14).** `GET /portfolio/{accountId}/positions/{pageId}` and `GET /iserver/account/trades` show the same first-touch-after-session-start sparseness §10.6 documents for live orders — sparse/field-missing position rows, an empty trades result — but neither carries a wire-reported freshness flag the way live orders' `snapshot: bool` does. The two endpoints get different treatments, governed by one principle: **the library auto-retries only on a wire shape that positively indicates incompleteness; it never retries on absence of data**, because absence is also a legitimate answer.

- **Positions — transparent retry-once.** When a read looks sparse (a *non-empty* list with a row missing `name`/`ticker` — the probe-pinned cold signature; an empty list never triggers it), `GetPositionsAsync` retries once, immediately, capped at one attempt. A retry HTTP failure keeps the first successful (sparse-but-valid) result rather than surfacing the retry's error (ADR-0009 point 4a). No public-surface change — no `IsSnapshot`-style wrapper; a retry is observable only via an `Activity` tag (`ibkr.cold_read_retry`). The retry is an ordinary request in the pipeline: it participates in standard 401 recovery like any idempotent GET, which composes with (and does not weaken) the concurrent-401 single-re-auth dedup — that guarantee collapses *concurrent* recoveries; a retry 401ing after an earlier recovery completed is a new failure and re-auths correctly (ADR-0009 point 5).
- **Trades — documented, no retry (consumer obligation).** `GetTradesAsync` makes one call and returns what IBKR returns. A first-of-session read may be `[]` despite trades existing, with no wire signal to distinguish it from a genuinely quiet day — the same inform-don't-act posture as §10.6's `IsSnapshot`, minus the flag IBKR doesn't provide here. A consumer that cares re-calls (the known live consumer already primes this endpoint itself). Deciding forces (ADR-0009 point 1a): an empty-list trigger misfires on every quiet-day poll, and the endpoint's own `1 req/5 secs` limiter (§8.1) turns each misfire into an up-to-~5s stall — a recurring tax, unlike Positions' precise row-level trigger on an un-throttled path.
