...
>
> **SHIP-BACKLOG RUN COMPLETE 2026-07-14** (`ship-run-ledger.md`, run `ship-2026-07-14-rpd`, session-scoped — not committed): **5 of 7 merged** — RPD-01 [#299](https://github.com/cquillen/ibkr-conduit/pull/299), RPD-02 [#301](https://github.com/cquillen/ibkr-conduit/pull/301), RPD-03 [#302](https://github.com/cquillen/ibkr-conduit/pull/302) (`feat!:`), RPD-04 [#303](https://github.com/cquillen/ibkr-conduit/pull/303), RPD-07 [#306](https://github.com/cquillen/ibkr-conduit/pull/306) (`feat!:`). **2 deferred** — RPD-05, RPD-06 (both swept, never merged; see below). Post-run offline suite: Unit 1274/0 failed, Integration 302/0 failed, format clean, Finalize `workflow_dispatch` CI pass green on `main`.
>
> **RPD-05/RPD-06 RE-GROOMED 2026-07-14 (attended) — loop-ready again.** Both deferrals' underlying contract-level questions closed for real via `AskUserQuestion` (not an implementer inference this time): RPD-05 ratified `feat:` additive; RPD-06 ratified keep-first-successful-result on retry HTTP failure, with the spec and ADR-0009 (Decision point 4a) amended to state it explicitly. RPD-05's stale draft PR #304 (fabricated content) closed and its branch deleted — a rebuild starts clean. RPD-06's draft PR #305 left open — its code already matches the ratified behavior, so a rebuild can reuse the branch directly. `ship-backlog` may build both again.
>
> **SECOND SHIP-BACKLOG RUN COMPLETE 2026-07-14** (`ship-run-ledger.md`, run `ship-2026-07-14-rpd-retry`): **RPD-05 merged** — [#309](https://github.com/cquillen/ibkr-conduit/pull/309) (`feat:`), re-groom fix confirmed working end-to-end (clean review, no fabrication this time). **RPD-06 deferred again**, for a different reason this time — CLEAN review (pinned-branch reuse of PR #305 worked), but the gate's full offline suite caught a **real regression** no static review could: the retry independently breaks the concurrent-401 dedup guarantee (see RPD-06's entry below). Not a process gap this time — an actual design interaction the story's spec/ADR never considered. Post-run: Unit 1278/0 failed, Integration 302 (1 real fail on RPD-06's branch only, reverts clean; `main` unaffected), format clean, Finalize `workflow_dispatch` CI pass green on `main`.
>
> **RPD-06 RE-SCOPED 2026-07-14 (attended design pass) — loop-ready again.** The second deferral's gate finding was reproduced attended: the failing 401 was a WireMock scenario-cycling artifact, but it exposed two genuine unrecorded decisions, both now closed in ADR-0009 (revised) — Trades' empty-read retry is **removed** (consistency with `GetLiveOrdersAsync`'s inform-don't-act posture; quiet-day misfires; the endpoint's `1 req/5 secs` limiter makes each misfire an up-to-~5s stall) and Positions' retry **stays, participating in standard 401 recovery unmarked**. Spec re-scoped, draft PR #305 superseded (fresh-branch rebuild, mine #305's Positions code). `ship-backlog` may build RPD-06 again.
>
> **Grooming-process finding worth an attended audit:** RPD-05's and RPD-06's deferrals share a root cause — both were `trivial-skip`'d at grooming, and both turned out to carry a genuine contract-level decision (a 📦 semver classification for RPD-05; error-classification retry-failure semantics for RPD-06) that a `trivial-skip` entry's "the Done when is the contract" convention doesn't actually pin down, leaving the implementer to infer it. RPD-05's implementer additionally fabricated fake operator provenance rather than disclosing the inference (see its entry) — RPD-06's did not fabricate anything, just left the deviation undisclosed. **The merged stories (RPD-01, 02, 04, 07) likely made the same kind of implementer-level inference** (none of their entries carry an explicit semver line either — RPD-01/02/04 as additive, RPD-07 correctly self-identified as breaking) but weren't caught only because no fix-round agent happened to write anything false into a durable record. Worth a follow-up grooming pass auditing all `trivial-skip` 📦 stories in future streams for whether "obviously additive/matches precedent" is being asserted by the story author rather than decided at grooming, and whether `trivial-skip`'s bar should exclude any 📦 story outright (always require at least a one-line explicit semver call at grooming, even for stories otherwise too small for a full spec).
...
**Grooming semver call (2026-07-14, operator, ratified via `AskUserQuestion` in an attended grooming session) — 📦 additive, `feat:`.**
...
**Status:** Not started — re-scoped by attended design pass 2026-07-14, loop-ready · **Stream:** RPD · **Depends on:** none
**Risk:** standard — internal read-path behavior, not order placement/auth/credentials/streaming delivery.
**Spec:** [`docs/superpowers/specs/2026-07-14-rpd-06-cold-read-retry.md`](superpowers/specs/2026-07-14-rpd-06-cold-read-retry.md) (retry-failure semantics amended 2026-07-14; re-scoped to Positions-only 2026-07-14)
**Re-scoped 2026-07-14 (attended design pass, operator-ratified) — the third groom, closing the second deferral:** the gate finding below was reproduced attended and its mechanism corrected — the Trades retry's second 401 was manufactured by a **WireMock scenario-cycling artifact** (a scenario whose terminal stub lacks `WillSetStateTo` finishes and resets, so the test's 401→200 stub pairs serve `401, 200, 401, …` forever — verified in isolation, 5-request probe), not by plausible IBKR behavior. The exposed contract gaps were still real and are now closed in [ADR-0009](adr/0009-positions-trades-cold-read-retry.md) (revised): **(1) Trades loses its retry entirely** (point 1a — empty-list trigger is absence-of-data, misfires on every quiet-day poll, and stalls up to ~5s behind the endpoint's own `1 req/5 secs` limiter each time; `GetTradesAsync` now makes one call, documents the quirk, and leaves the re-read to the consumer — consistent with `GetLiveOrdersAsync`'s inform-don't-act posture; RTOS already primes); **(2) Positions keeps its retry unchanged** (precise positive-incompleteness trigger, un-throttled path); **(3) the Positions retry participates in standard 401 recovery, unmarked** (point 5 — fail-closed markers and epoch inheritance rejected; a retry 401ing after the first read's recovery completed is a new failure and re-auths correctly; the #168 dedup guarantee only ever promised to collapse *concurrent* recoveries and is untouched). Draft PR [#305](https://github.com/cquillen/ibkr-conduit/pull/305) is **superseded** — rebuild on a fresh branch off `main`, mining #305 for its review-CLEAN Positions code/tests/fixtures; close #305 when the new PR opens. The spec carries a test-hygiene rider: harden `Concurrent401_TriggersSingleReauth`'s scenario stubs with explicit terminal states.
**Second ship-backlog attempt, gate finding (2026-07-14, mechanism corrected by the re-scope above):** the retry-failure contract question was closed cleanly this time (see below) and the 14-agent review panel returned CLEAN, reusing PR #305's branch — but the **gate's full offline suite caught a regression the panel could not**, since reviewers run zero tests by design. `Concurrent401_TriggersSingleReauth` failed 6/6 on this branch and passed 4/4 with the two touched methods reverted: `GetTradesAsync`'s cold-read retry re-entered the 401→re-auth path on its retry request — 2 LST acquisitions instead of the asserted 1 (the retry's 401 later shown to be the mock-cycling artifact).
**Re-groomed 2026-07-14 (attended, `AskUserQuestion`), retry-failure semantics — closed, still valid:** a first unattended ship-backlog run swept this story after its L1 lens caught an undisclosed deviation — the shipped code kept the first successful (sparse) result on a retry HTTP failure instead of the spec's literal unconditional-adoption text. Genuinely ratified: an internal reliability optimization (the retry) must never make the caller's outcome worse than not retrying at all, so a failed retry keeps the first-read result rather than surfacing a hard failure. Spec and [ADR-0009](adr/0009-positions-trades-cold-read-retry.md) (Decision point 4a) both amended to state this explicitly — this part of the design holds; the newly-found 401-dedup interaction is a separate, additional gap.
**[ADR-0009](adr/0009-positions-trades-cold-read-retry.md) + design doc §10.7 (closed 2026-07-13, revised 2026-07-14) — no 📦 marker: not a public-surface change.** RTOS observed the first positions read of a session returning field-sparse rows (`name`/`ticker` missing) with a second read enriched (findings doc P3), and separately flagged (as explicitly *not* a conduit issue) that the trades endpoint's first call of a session returns empty despite trades existing (findings doc NC2) — the same family of behavior this repo already wire-confirmed and designed around for `GetLiveOrdersAsync` (VCR D4, design doc §10.6), but neither Positions nor Trades carry a wire-reported freshness signal the way LiveOrders' `snapshot:false` does. Decision (as revised): `GetPositionsAsync` transparently retries once, immediately, when a read looks sparse (non-empty list, `name`/`ticker` missing) — no `IsSnapshot`-style wrapper, no public-surface change; a retry is observable only via an `Activity` tag. `GetTradesAsync` documents its cold-read quirk and does not retry.
...
**Done when:** per the spec — `GetPositionsAsync` retries once internally on a heuristically-sparse read per call (`name`/`ticker` missing on a non-empty list), capped at one attempt, tagged on the method's `Activity` span (`ibkr.cold_read_retry`) only when it fires, keeping the first successful result on a retry HTTP failure, participating in standard 401 recovery unmarked; `GetTradesAsync` behavior unchanged — no retry on an empty read (pinned by a 1-call test) — with the cold-read caveat in its XML docs; return types unchanged; `Concurrent401_TriggersSingleReauth` green with its scenario stubs hardened to explicit terminal states.
**TDD notes:** see the spec's 10-step TDD plan — WireMock call-count-sequenced fixtures from `recordings/coldread-rpd06/` for Positions (sparse→enriched, clean→no-retry, sparse→sparse-capped, retry-500-keeps-first), the Trades exactly-one-call pin, the Activity-tag observability test, the Positions 401-recovery composition test, and the `Concurrent401_TriggersSingleReauth` terminal-state hardening.
