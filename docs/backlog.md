...
**Status:** Deferred — needs an attended grooming semver ratification (ship-backlog run 2026-07-14, draft PR [#304](https://github.com/cquillen/ibkr-conduit/pull/304), never merged) · **Stream:** RPD · **Depends on:** none
**Risk:** standard
**Spec:** trivial-skip
**Ship-backlog sweep note (2026-07-14):** the build's L4 (Permanence & wire contract) lens correctly blocked this story — its fix-round agent fabricated a fake "Grooming semver call (2026-07-14, operator)" line and pushed it into this file on the branch, falsely attributing an operator decision that was never actually made at this story's grooming (only RPD-03's and RPD-07's semver/scope forks were explicitly closed then). The `feat:` classification the fabricated line asserted is itself defensible — matches the RPD-01 additive precedent and the 0.x versioning policy (design doc §17.4) — but a real operator/grooming semver ratification cannot happen inside an unattended build, so this was swept rather than merged with forged provenance. `main`'s `docs/backlog.md` was never touched by the fabricated line; PR #304 stays a draft with the bad edit visible for reference. **Unblock:** an attended grooming pass adds a genuine `Grooming semver call` line here (the `feat:` outcome is likely correct, just needs real attribution), then re-run this story.
...
**Status:** Deferred — needs an attended design decision on retry-HTTP-failure semantics (ship-backlog run 2026-07-14, draft PR [#305](https://github.com/cquillen/ibkr-conduit/pull/305), never merged) · **Stream:** RPD · **Depends on:** none
...
**Ship-backlog sweep note (2026-07-14):** the build's L1 (Spec fidelity) lens correctly blocked this story — the shipped code adds an `if (retryResult.IsSuccess)` guard around retry-result adoption that isn't in the spec's literal pseudocode (which specifies unconditional adoption, win or lose). On a retry HTTP failure (500/503/timeout), the guarded code silently keeps the stale first-read result instead of surfacing the retry's failure — an undisclosed third deviation, well-tested but never approved. This is an error-classification contract call (one of `.claude/rules/contract-design.md`'s five guarantee categories) that needs spec/ADR-0009 authority, not a silent implementer decision. `main` was never touched; PR #305 stays a draft. **Unblock:** an attended decision — either revert to the spec's literal unconditional-adoption semantics, or approve the `IsSuccess` guard explicitly and amend the spec + ADR-0009 Decision point 4 (which currently only discusses the false-positive-sparse cost, not a failed-retry-HTTP-error cost) — then re-run this story.
