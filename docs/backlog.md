...
**Status:** ✅ Done — #304 · **Stream:** RPD · **Depends on:** none
**Risk:** standard
**Spec:** trivial-skip
...
**Grooming semver call (2026-07-14, operator) — 📦 additive, `feat:`.** All nine fields move from the `[JsonExtensionData]` bag to typed positional-record properties, growing `Position`'s primary-ctor/`Deconstruct` arity by nine — the same trailing-optional-nullable promotion pattern RPD-01 already shipped additively (`d906fc5`, `feat: promote ... fields (RPD-01)`), and permitted outright under the 0.x policy (design doc §17.4: "public API may change between minor versions"). No existing property is renamed, retyped, or removed; a consumer reading these values via `AdditionalData` sees them move to strongly-typed properties, and a positional `Deconstruct`/binary-ctor call site breaks only if it pattern-matches on today's exact (narrower) arity — accepted, additive-by-convention risk, not a breaking one. Commit subject is `feat:`, not `feat!:`.
