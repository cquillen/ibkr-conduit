...
**Status:** Deferred — real gate regression, needs an attended design decision on retry-vs-401-dedup interaction (ship-backlog run 2026-07-14, second attempt; draft PR [#305](https://github.com/cquillen/ibkr-conduit/pull/305), never merged) · **Stream:** RPD · **Depends on:** none
...
**Second ship-backlog attempt, gate finding (2026-07-14):** the retry-failure contract question was closed cleanly this time (see below) and the 14-agent review panel returned CLEAN, reusing PR #305's branch — but the **gate's full offline suite caught a real regression the panel could not**, since reviewers run zero tests by design. `Concurrent401_TriggersSingleReauth` failed 6/6 on this branch and passed 4/4 with the two touched methods reverted — a flake-vs-real judgment agent traced the causal mechanism: `GetTradesAsync`'s cold-read retry, when it fires *after* the original request's 401-triggered recovery already completed, issues a second `/trades` request that independently re-enters the 401→re-auth path. That second request is no longer concurrent with the sibling request's 401, so `SessionManager`'s epoch-based dedup (`SessionManager.cs:308,331`) can't collapse it into the same LST acquisition — 2 acquisitions instead of the asserted 1. **Unblock:** an attended design decision on how the retry should interact with concurrent-401 dedup — candidates include having the retry inherit/share the original request's re-auth epoch, or having a retry that itself would need to 401-recover fail closed (return the first result) instead of re-entering the full re-auth path. This is a `docs/adr/0009-positions-trades-cold-read-retry.md`-level decision (error-classification / session-lifecycle interaction), not a mechanical fix — do not re-run this story without one.
