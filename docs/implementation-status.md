...
| Refit `11.2.0` → `12.1.0` | **Deferred** (see PR #206) | Refit 12 is a major rewrite (reflection-free, AOT-ready source generation). It is a clean recompile against our `IApiResponse` usage, but is **blocked for this multi-project codebase by two source-generator conflicts under `InternalsVisibleTo`**: (1) the new `ReactiveUI.Primitives` 5.8.0 subtree (which replaces `System.Reactive` on the shipped dependency graph) ships an R3Bridge generator that always emits an internal `PrimitivesR3BridgeGeneratedAttribute` into every project → `CS0436` errors in `IbkrConduit.HealthChecks`; (2) even after stripping that generator, Refit 12's `InterfaceStubGeneratorV2` emits `PreserveAttribute`/`Generated` types that collide across the IVT boundary in `IbkrConduit.Tests.Unit` (10× `CS0436` — see [reactiveui/refit#565](https://github.com/reactiveui/refit/issues/565)). Also requires `Microsoft.Extensions.Http` ≥ 10.0.9 (satisfied by the low-risk batch). **Remediation (own task):** resolve the generated-marker collisions per refit#565 (namespace/visibility of the emitted stubs, or narrowing the affected `InternalsVisibleTo`), decide on the `ReactiveUI.Primitives` dependency, and add a serializer test for Refit 12's new default `NumberHandling = AllowReadingFromString`. |
