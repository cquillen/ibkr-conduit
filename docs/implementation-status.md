...
| 5.8 | WebSocket unsubscribe (u… topics) + subscription handles | Done |
...
| Low-risk batch (0.5.3) | Done | Routine minor/patch bumps validated together and shipped as one PR: `Refit`/`Refit.HttpClientFactory` → 11.2.0, all `Microsoft.Extensions.*` → 10.0.9, `Polly.Core` → 8.7.0, `System.Threading.RateLimiting` → 10.0.9, `Microsoft.Extensions.TimeProvider.Testing` → 10.7.0, `Spectre.Console` → 0.57.1, `WireMock.Net` → 2.11.0, plus CI `actions/checkout` v6 → v7 and `codecov/codecov-action` v6 → v7. Full build + 724 unit + 232 integration tests green. |
| Refit `11.2.0` → `12.1.0` | Done | Refit 12 is a major rewrite (reflection-free, AOT-ready source generation). A clean recompile against our `IApiResponse` usage, but it surfaced source-generator conflicts under `InternalsVisibleTo`, resolved in `Directory.Build.targets` (imported by every project, including the test projects that don't chain the root `Directory.Build.props`): **(1)** Refit's `InterfaceStubGeneratorV2` emits an internal `PreserveAttribute` whose namespace is `$(RefitInternalNamespace)` — defaulted to `$(RootNamespace)`, which is empty when `refit.props` imports, so the stubs landed in the global namespace and clashed across IVT. Fixed by setting `RefitInternalNamespace` = `$(MSBuildProjectName)` (per-assembly unique; this is the exact case `refit.props` documents). **(2)** The new `ReactiveUI.Primitives` 5.8.0 subtree (replacing `System.Reactive`) ships an R3Bridge generator that always emits an internal `PrimitivesR3BridgeGeneratedAttribute` into every project → `CS0436` in `IbkrConduit.HealthChecks`. We never use the R3 bridge (all Refit methods return `Task<IApiResponse<T>>`), so an MSBuild target removes that generator from the compile. **(3)** Refit also emits a fixed-namespace `Refit.Implementation.Generated` module-initializer it does not let us rename; the benign IVT duplication of that one type is suppressed via `NoWarn CS0436` scoped to `IbkrConduit.Tests.Unit` only. Refit 12's new default `NumberHandling = AllowReadingFromString` (which the library relies on via `AddRefitClient<TApi>()` with no custom `RefitSettings`) is guarded by `RefitSerializerDefaultsTests`. Test fakes updated for Refit 12's requirement that `ApiResponse<T>` have an associated request message. |
...
**Goal:** Close the money-boundary defects found by the [2026-07-04 RTOS venue-consumer review](findings/2026-07-04-rtos-venue-consumer-review.md) before/alongside the RTOS live-paper soak.

**Tracker: [Stream VCR in docs/backlog.md](backlog.md)** — groomed loop-ready 2026-07-07 (PR #236); story status lives **only** there (single source of truth per `.claude/rules/backlog-status.md`, updated in each story's own PR — not mirrored here). Decisions: [ADR-0001..0004](adr/README.md) + design doc §6.5/§7.7/§9.9/§10.6/§12.8. The five breaking stories (VCR-01/02/04/05/07, `feat!:`) ship as one release train (the **0.9.0** cut); the rest are `fix:`.

> The original pre-pipeline tracker for this milestone, [money-boundary-hardening-backlog.md](money-boundary-hardening-backlog.md) (Stream MBH, with its [design spec](superpowers/specs/2026-07-04-money-boundary-hardening-backlog-design.md)), was **retired 2026-07-07** in favor of Stream VCR — no MBH task was started; it is retained as history only.

| Story | Title | Findings |
|---|---|---|
| VCR-01 📦 | Presence-preserving wire DTOs (streaming + REST) | WIR-1, WIR-3, FIL-6, GAP2-2, GAP2-3 |
| VCR-02 📦 | Streaming delivery observability & subscription semantics | FIL-1, GAP2-4, FIL-3, FIL-4, FIL-5 |
| VCR-03 | Streaming mapper robustness | FIL-2, GAP2-1 |
| VCR-04 📦 | Order-outcome classification & 401 replay gate | AMB-2, AMB-3, AMB-4, WIR-4 |
| VCR-05 📦 | Live-orders priming & filters/sor interaction | GAP1-1, GAP1-2, GAP1-3 |
| VCR-06 | Session lifecycle state-machine hardening | SES-2, SES-3, SES-5, SES-6 |
| VCR-07 📦 | Competing-session truth & health evidence | SES-1, GAP3-1, GAP3-2, GAP3-3, SES-4 |
| VCR-08 | Manager lifecycle integrity | MGR-1, MGR-2, MGR-3, MGR-6 |
| VCR-09 | Metrics registration & disposal hygiene | MGR-4, FIL-7, MGR-5 |
| VCR-10 | Response-schema validation net hardening | WIR-5 |
| VCR-11 | Order-type documentation vs captured wire enum | WIR-6 |
| VCR-12 | ExtOperator futures-compliance field *(Deferred)* | WIR-6 (follow-on) |
