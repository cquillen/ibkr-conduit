...
3. **Otherwise, classify every element independently, preserving wire order** (never reorder, never key on index). The `OrderSubmitted` classification is **positive/allowlist-gated**, not blocklist-gated, so an unknown status fails toward the safe direction even when an `order_id` is present:
   - `element.Error is not null` → `IbkrOrderRejectedError(element.Error, rawBody, requestPath)`.
   - Else `element.OrderStatus` is in the **known** terminal non-transmitting set (see below) → `IbkrOrderRejectedError($"Order not transmitted (status: {element.OrderStatus})", rawBody, requestPath)`, **even when `element.OrderId` is a real-looking value** — this upgrades a *known-bad* status from ambiguous to a definite rejection (a rejected parent must never surface as `OrderSubmitted` just because it carries an `order_id`).
   - Else `element.OrderId` is a **real (positive) order id** AND `element.OrderStatus` is in the **transmitting allowlist** (see below) → `OrderSubmitted(element.OrderId, element.OrderStatus, element.LocalOrderId, element.OcaGroupId)`. This is the *only* path to `OrderSubmitted`.
   - Else `element.OrderId is not null` (a sentinel/non-positive `order_id`, or a real `order_id` under a status **not** recognized as transmitting — e.g. an unobserved terminal value like `"Cancelled"`/`"Rejected"`) → `IbkrAmbiguousOrderError` — the safe direction: something may have been sent, outcome unknown, reconcile, never re-place.
   - Else (no `order_id` and matching none of the above — defensive, not wire-observed) → `IbkrAmbiguousOrderError(null, "Unrecognized order-leg response shape", rawBody, requestPath, ReauthSucceeded: false)`.
...
**Transmitting `order_status` allowlist** (internal constant — the actual money-safety boundary): `"PreSubmitted"`, `"PendingSubmit"`, `"Submitted"` (wire-observed), plus `"Filled"`, `"Modified"` (the other documented working states). A leg reaches `OrderSubmitted` **only** when it carries a positive `order_id` AND a status in this set; every other `order_id`-bearing row degrades to `IbkrAmbiguousOrderError`. Because IBKR's status vocabulary is not closed, this positive gate — rather than a blocklist of bad statuses — is what guarantees an *unrecognized* future status never silently misclassifies as success; it degrades to ambiguous, the safe direction.

**Known terminal non-transmitting `order_status` set** (internal constant): `"Failed"`, `"Inactive"`. Wire-observed values only. This set is **not** the safety boundary (the transmitting allowlist is); it only *upgrades* a known-bad status from the ambiguous fallback to a *definite* `IbkrOrderRejectedError`, so the common rejected-parent shape reports as a clear rejection rather than merely-ambiguous.

**Real vs. sentinel `order_id`:** an `order_id` is real only when it parses to a positive integer. IBKR's rejection sentinel `"-1"` (and any non-positive/non-numeric value) is never a transmitted order identity, so it never gates an `OrderSubmitted` however live-looking its `order_status` is (design doc §9.11's "real vs. sentinel order_id" signal).
