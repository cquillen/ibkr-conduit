...
- **DOC-08 extras probed (paper account, 2026-07-07 ~18:14 UTC — inside the close-order window — `recordings/ordertype-probe-2026-07-07.log`, operator-acked mutating probe, 1 sample per type, SPY conid 756733): all five accepted and live.** `LOC` (question `o354` → reply → `order_id 892117170`, PreSubmitted), `MOC` (892117172), `MIT` (892117174), `LIT` (892117175), `REL` (auxPrice −50, 892117176) — none rejected as invalid orderType, none rejected for timing/venue; all cancelled and verified (0 fills, 0 working orders left). Per-sample, not always-accepted: one instrument, one account state, one time window.
- Newly observed **submission-token → read-side-name mapping** (same probe, undocumented anywhere): `LOC→LIMITONCLOSE`, `MOC→MARKETONCLOSE`, `REL→RELATIVE`, `MIT→MIT`, `LIT→LIT` — matching DOC-01's lower_snake `contractRules.orderTypes` namespace (`limitonclose, marketonclose, relative, mit, lit`), which the docs never connect to the submission codes.
- The full admissible enum still has no wire pin (you cannot enumerate an enum by probing); the claim tier is DOC-03's list plus DOC-08's examples, now with 5/5 of the DOC-08 extras wire-accepted.
...
- **Conflicts:** **the admissible-enum claim was cross-source contradicted — and resolved by the wire (2026-07-07, operator-acked probe).** DOC-03's place-order section states a closed seven-value list ("Available Order Types: LMT, MKT, STP, STOP_LIMIT, MIDPRICE, TRAIL, TRAILLMT"); DOC-08 — the page DOC-03 itself defers to for advanced order types — shows CP API worked examples submitting `LOC`, `MOC`, `MIT`, `LIT`, `REL`; the probe accepted **all five** (see Wire observations). DOC-03's list is wire-falsified *as a closed set* — treat it as the documented core. Consequence: VCR-11's shipped XML docs (`IIbkrOrderApiModels.cs:24,:106`) are under-inclusive — remedied by **VCR-13** (doc-widening story, drafted from this evidence; no PVR design depends on the enum being closed — PVR-05's fail-fast keys on TRAIL/TRAILLMT presence only). One **intra-source** contradiction: DOC-03's `trailingAmt` "Required … optional if" wording; its unambiguous "must specify both" sentence and the wire acceptance carry the decision.
...
- **Presence claims:**
  - Seven-value submission enum: **documented (DOC-03 only), contradicted-by-example (DOC-08), falsified-as-closed by the wire.** TRAIL observed accepted (1 sample); LMT/MKT/STP/STOP_LIMIT exercised throughout this repo's recordings; MIDPRICE/TRAILLMT no wire sample; the DOC-08 extras (LOC/MOC/MIT/LIT/REL) **documented-by-example + observed accepted (1 sample each)**.
...
**Answer for the consuming decisions (PVR-05 §9.7; VCR-11 historical; VCR-13):** the groomed design stands unchanged — add `TrailingAmt`/`TrailingType` with client-side fail-fast when `TRAIL`/`TRAILLMT` lacks them, values `"amt"`/`"%"`. The enum pin migrates from the deprecated mirror citation to DOC-03 (live, 2026-07-07) with wire support for TRAIL. The DOC-08 conflict is **probe-resolved**: LOC/MOC/MIT/LIT/REL are wire-accepted, so DOC-03's seven-value list is the documented core, not a boundary — VCR-13 widens the shipped XML docs accordingly (values + per-sample caveat + the read-side name mapping above).
